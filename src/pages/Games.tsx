      ...
      <Card className="gaming-card group overflow-hidden flex flex-col h-full">
        ...
        <CardContent className="space-y-4 flex flex-col flex-grow justify-between">
          <div className="flex flex-wrap gap-1 items-center transition-all duration-300">
            ...
            {game.tags && game.tags.length > 3 && !isExpanded && <Badge variant="secondary" className="text-xs cursor-pointer hover:bg-primary/20" onClick={() => setIsExpanded(true)}>+{game.tags.length - 3}</Badge>}
          </div>
          <Link to={`/games/${game.id}`} className="mt-auto"><Button variant="gaming" className="w-full">Learn More</Button></Link>
        </CardContent>
      </Card>
    );
  };
  ...
  <meta name="description" content="Explore the full collection of games from DSY Studio. Discover our immersive gaming experiences, from psychological horror to cyberpunk adventures." />
      ...
      <div className="container mx-auto px-4 pt-32 pb-16">
        ...
        <Tabs defaultValue="featured" className="w-full">
          <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 mb-8"><TabsTrigger value="featured" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">Featured</TabsTrigger><TabsTrigger value="all" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">All Games</TabsTrigger></TabsList>
          <TabsContent value="featured" className="space-y-8"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuredGames.map((game, index) => <div key={game.id} className="animate-fade-in-scale" style={{ animationDelay: `${index * 0.1}s` }}><GameCard game={game} /></div>)}
          </div></TabsContent>
          <TabsContent value="all" className="space-y-8"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {allGames.map((game, index) => <div key={game.id} className="animate-fade-in-scale" style={{ animationDelay: `${index * 0.1}s` }}><GameCard game={game} /></div>)}
          </div></TabsContent>
        </Tabs>
      </div>
    </div>
